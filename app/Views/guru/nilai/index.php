...
<div class="flex justify-between items-center mb-6">
    <div>
        <h3 class="text-xl font-bold text-slate-800">Input Nilai Santri</h3>
        <p class="text-sm text-slate-500 mt-1">Pilih kelas untuk melakukan penilaian santri.</p>
    </div>
</div>
    ...
    <?php else : ?>
        <?php foreach ($classes as $c) : ?>
            <div class="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm hover:shadow-md transition-all group">
                ...
                <a href="<?= base_url('guru/nilai/input/' . $c['id']) ?>" class="w-full bg-slate-50 hover:bg-blue-600 hover:text-white text-slate-600 font-bold py-3 rounded-xl transition-all flex items-center justify-center space-x-2">
